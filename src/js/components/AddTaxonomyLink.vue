<template>
    <div :id="`posttype-${taxonomy.slug}`" class="posttypediv" v-if="menuItems.length">

        <ul :id="`posttype-${taxonomy.slug}-tabs`" class="posttype-tabs add-menu-item-tabs">
        ...
        <div :class="{['tabs-panel-active']: currentTab === 'all', ['tabs-panel-inactive']: currentTab !== 'all'}"
            ...
            <ul :id="`${taxonomy.slug}checklist`" class="categorychecklist form-no-clear">
        ...
        <div :class="{['tabs-panel-active']: currentTab === 'search', ['tabs-panel-inactive']: currentTab !== 'search'}"
            ...
            <form v-on:submit.prevent="loadItems">
                <p class="quick-search-wrap">
                    <label :for="`quick-search-posttype-${taxonomy.slug}`" class="screen-reader-text">Search</label>
                    <input :id="`quick-search-posttype-${taxonomy.slug}`" type="search" class="quick-search"
                           v-model="search" @keyup="loadItems"/>
                    <span class="spinner" v-if="showSpinner"></span>
                </p>
            </form>

            <ul :id="`${taxonomy.slug}-search-checklist`" class="categorychecklist form-no-clear">
    ...
    export default {
        ...
        methods: {
            loadItems() {
                ...
                this.fetch({
                    path: `/wp/v2/${this.taxonomy.rest_base}${queryString}`,
                    parse: false,
                })
                    .then((response) => {
            ...
            transformItem(term) {
                return {
                    object: this.taxonomy.slug,
                    object_id: term.id,
                    post_status: 'draft',
                    post_title: term.name,
                    title: term.name,
                    type: 'taxonomy',
                    type_title: this.taxonomy.name,
                };
            }
        },
        watch: {
